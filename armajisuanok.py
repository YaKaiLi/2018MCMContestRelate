...
if __name__ == '__main__':
    ...
    sql = "select data from `sesedsneed_copy` WHERE `MSN` = 'NUETB' AND `statecode` = 'CA' ORDER BY `year`"
    ...
    plt.show()
    '''
    model = pf.ARIMA(data=total_diff,ar=4,ma=4,integ=0,target='diff')
    x = model.fit("MLE")
    predict_diff = model.predict(h=40)
    predict_diff.iloc[0] = predict_diff.iloc[0] + total[0][49]
